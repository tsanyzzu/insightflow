...
# Navigasi Tab
tab1, tab2, tab3 = st.tabs(["0️⃣ Quant Scan (Context)", "2️⃣ Narrative Radar (Trends)", "3️⃣ Deep Dive Analyzer"])
...
with tab2:
    ...
    if st.button("SEARCH NARRATIVES 🕵️", key="btn_narrative"):
                ...
                for idx, item in enumerate(narratives):
                    with cols[idx % 3]: 
                        with st.container(border=True):
                            ...
                            if "Exploding" in velocity: st.markdown("🔥 **Exploding**")
                            elif "Rising" in velocity: st.markdown("📈 **Rising**")
                            else: st.markdown(f"➡️ {velocity}")
